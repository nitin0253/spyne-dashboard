...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Sanitize strings to prevent malformed JSON from special chars in cell values
    // (newlines, tabs, null bytes from Google Sheets cells break JSON.stringify)
    function sanitize(v) {
      if (typeof v === 'string') {
        return v
          .replace(/\0/g, '')
    ...
    const payload = sanitize({ months, errors, enterpriseMeta, fetchedAt: new Date().toISOString() });
    let jsonStr;
    try {
      jsonStr = JSON.stringify(payload);
    } catch (e) {
      console.error('[data.js] JSON.stringify failed:', e.message);
      jsonStr = JSON.stringify({ months: sanitize(months), errors, enterpriseMeta: {}, fetchedAt: new Date().toISOString() });
    }
